...
public class RestServerMockContainerExtension extends ToggleableContainerExtension
  ...
  private MockServerContainer mockRestServer =
      new MockServerContainer(DockerImageName.parse("mockserver/mockserver").withTag("5.15.0"));
  ...
  public void beforeAllInternal(ExtensionContext context) {
    ...
    log.info(
        String.format(
            "### REST mock server container startup finished with properties: [MOCK_URL=%, MOCK_WORKFLOWS_URL=%]",
            mockUrl, workflowsUrl));
  }
  ...
  public void afterAllInternal(ExtensionContext context) {
    ...
    var recordedRequests =
        restMockClient.retrieveRecordedRequests(HttpRequest.request(), Format.JSON);
    log.info("### Recorded requests: {}", recordedRequests);
    var recordedExpectations =
        restMockClient.retrieveRecordedExpectations(HttpRequest.request(), Format.JSON);
    log.info("### Recorded expectations: {}", recordedExpectations);
    var activeExpectations =
        restMockClient.retrieveActiveExpectations(HttpRequest.request(), Format.JSON);
  ...
  @Override
  public MockServerClient resolveParameter(
      ParameterContext parameterContext, ExtensionContext extensionContext)
      throws ParameterResolutionException {
  ...
  private void setConnectionProperties() {
    System.setProperty(
        MOCK_URL,
        "http://localhost:" + mockRestServer.getHost() + ":" + mockRestServer.getServerPort());
    System.setProperty(
        MOCK_WORKFLOWS_URL,
        "http://localhost:" + mockRestServer.getHost() + ":" + mockRestServer.getServerPort());
